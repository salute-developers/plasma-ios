...
| `SDDSIcons.xcframework.zip` | `build/` |
...
2. `ruby scripts/build_xcframeworks.rb -d . -w SDDS.xcworkspace` (InputMask, SDDSComponents, SDDSIcons)
...
6. Зип xcframework'ов из `build/` и `DesignSystemBuilder/build/` через
   `ditto -c -k --sequesterRsrc --keepParent`, копирование зипов тем.
7. Проверка: все файлы из контракта присутствуют в `<output>` (по умолчанию
   ...
   Список тем вычисляется динамически по `Themes/*/*.xcodeproj` (схема = имя проекта).
...
Токен — `secrets.GH_TOKEN`, как сейчас. `permissions: contents: write`.

### Удаляемое

`release_components.yml`, `release_icons.yml`, `publish-all-themes-release.yml`,
`release_all_frameworks.yml`, `scripts/dispatch-publish.sh`, `scripts/build_artifacts.rb`.
...
1. Локально: `scripts/release/build_release.sh release-test` под Xcode 26.6 → каталог
   `release-artifacts/` содержит все файлы контракта; `dsbuilder --help` из распакованного
