...
- Asset names are a contract: `SDDSThemeCore.xcframework.zip`, `InputMask.xcframework.zip`, `SDDSComponents.xcframework.zip`, `SDDSIcons.xcframework.zip`, `<Theme>.xcframework.zip` per `Themes/*/*.xcodeproj` (zip name follows the Xcode scheme, e.g. `SDDSServTheme` for project dir `SDDSservTheme`), `dsbuilder-cli-<tag>.zip`, `SDDSSources-<tag>.zip`.
...
- [ ] **Step 2: Make it executable, ignore the output dir, drop the duplicate**
...
- [ ] **Step 3: Argument handling smoke test (no build)**
...
- [ ] **Step 4: Full local run under Xcode 26.6**

Run: `scripts/release/build_release.sh release-local-test`
Expected: ends with `✅ Ассеты релиза release-local-test:` and the listing contains 4 core zips, 3 theme zips, `dsbuilder-cli-release-local-test.zip`, `SDDSSources-release-local-test.zip`.
...
**Files:**
- Delete: `.github/workflows/release_components.yml`, `.github/workflows/release_icons.yml`, `.github/workflows/publish-all-themes-release.yml`, `.github/workflows/release_all_frameworks.yml`, `scripts/dispatch-publish.sh`

- [ ] **Step 1: Delete**

```bash
git rm -q .github/workflows/release_components.yml .github/workflows/release_icons.yml \
...
- [ ] **Step 3: PR into the release branch**
