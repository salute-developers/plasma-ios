...
final class TextFieldViewModel: ObservableObject {
    ...
    @Published var appearance: TextFieldAppearance = SDDSComponents.TextField.default.medium.appearance
    
    var allTextFieldAppearance: [AppearanceVariation<TextFieldAppearance>] {
        switch layout {
        case .default:
            SDDSComponents.TextField.all.map { AppearanceVariation(name: $0.name, appearance: $0.appearance) }
        case .clear:
            TextFieldClear.all.map { AppearanceVariation(name: $0.name, appearance: $0.appearance) }
        }
    }
