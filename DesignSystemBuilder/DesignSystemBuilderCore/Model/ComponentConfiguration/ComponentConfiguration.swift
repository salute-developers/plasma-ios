...
struct ComponentBindingResolver {
    ...
    func resolve() -> ComponentBindingInfo? {
        ...
        // Все вариации с binding — включая родительские. Родительская вариация
        // (например базовая `xs` = required-placement none) сама является выбираемым
        // стилем, а не только контейнером для required-start/end дочерних вариаций.
        // Если брать только листья, комбинации с required-placement=none теряются
        // и binding-конфигуратор откатывается на fallback-стиль.
        let nodes = variations.filter { !$0.binding.isEmpty }

        // Тема без дерева вариаций (например HomeDS: FormItem — один дефолт с
        // единственной размерностью `view`). Синтезируем базовый узел `default`,
        // чтобы сгенерировать стили из base × значения view (accessor
        // `FormItem.default.<view>`).
        let effectiveNodes: [Variation] = nodes.isEmpty
            ? [Variation(id: "default", parent: nil, binding: [])]
            : nodes

        var styles: [ComponentBindingInfo.Style] = []
        var seen = Set<String>()

        for leaf in effectiveNodes {
...
extension ComponentBindingInfo {
    ...
    func stylesCollectionSource() -> String {
        ...
        for dim in paramDimensions {
            ...
            // Маппинг raw-строки (из binding-конфигуратора) в case — аналог Android
            // enum-mapping. Возвращает nil для неизвестного значения, чтобы вызывающая
            // сторона откатилась на дефолт.
            out += "\n        public init?(raw: String) {\n"
            out += "            switch raw {\n"
            for value in dim.values {
                out += "            case \"\(value.raw)\": self = .\(valueCase(value.raw))\n"
            }
        ...
        // Неподдерживаемая комбинация размерностей откатывается на первый стиль,
        // чтобы binding-конфигуратор не крашился на несуществующих сочетаниях.
        if let fallback = styles.first {
            out += "        default: return .\(styleCase(fallback))\n"
        } else {
        ...
        out += bindingApiSource()
        out += "}\n"
        return out
    }

    /// Генерирует binding-мост для generic-конфигуратора демо (аналог Android
    /// `bindings: Set<Property>` + `resolveStyleKey(Map)`):
    /// - `bindingProperties` — список «ручек» (size, view, …) со значениями и дефолтами;
    /// - `resolveStyle(bindings:)` — резолв стиля по stringly-typed map значений.
    private func bindingApiSource() -> String {
        var out = "\n    /// Список настраиваемых «ручек» компонента для binding-конфигуратора.\n"
        out += "    static let bindingProperties: [BindingProperty] = [\n"
        for dim in dimensions {
            let def = dim.defaultValue ?? dim.values.first?.raw ?? ""
            switch dim.type {
            case .boolean:
                out += "        BindingProperty(name: \"\(dim.name)\", kind: .toggle, defaultValue: \"\(def)\"),\n"
            case .enum, .view:
                let values = dim.values.map { "\"\($0.raw)\"" }.joined(separator: ", ")
                out += "        BindingProperty(name: \"\(dim.name)\", kind: .singleChoice([\(values)]), defaultValue: \"\(def)\"),\n"
            }
        }
        out += "    ]\n\n"

        out += "    /// Резолвит стиль по значениям binding-«ручек» (raw-строки).\n"
        out += "    /// Неизвестные/пустые значения размерности берут дефолт.\n"
        out += "    static func resolveStyle(bindings: [String: String]) -> \(returnType) {\n"
        for dim in paramDimensions {
            let def = dim.defaultValue ?? dim.values.first?.raw ?? ""
            out += "        let \(dim.paramName) = \(dim.enumTypeName)(raw: bindings[\"\(dim.name)\"] ?? \"\") ?? .\(valueCase(def))\n"
        }
        let forward = paramDimensions.map { "\($0.paramName): \($0.paramName)" }.joined(separator: ", ")
        out += "        return style(\(forward))\n"
        out += "    }\n"
        return out
    }
}
...
struct ConfigInfo: Codable {
    ...
    struct ComponentMeta: Codable {
        ...
        let props: [Prop]
        /// Заполняется только для компонентов, у которых реально сгенерирован
        /// Swift-тип `<Component>Styles` (см. `supportsBinding`). Иначе примеры в
        /// документации ссылались бы на несуществующие типы.
...
/// Валидация config-info перед записью: пустой `key` (или другое обязательное
/// поле) валит весь артефакт на стороне Documentation Service, поэтому ловим
/// это на генерации, с указанием виновного компонента.
enum ConfigInfoValidator {
    static func validate(componentsWithKeys keys: [String]) -> [String] {
        keys.enumerated().compactMap { index, key in
            key.trimmingCharacters(in: .whitespaces).isEmpty
                ? "component #\(index): пустой key — артефакт будет отвергнут сервисом"
                : nil
        }
    }

    static func validate(_ info: ConfigInfo) -> [String] {
        var issues = validate(componentsWithKeys: info.components.map { $0.key })
        for component in info.components {
            if component.coreName.isEmpty { issues.append("\(component.key): пустой coreName") }
            if component.styleName.isEmpty { issues.append("\(component.key): пустой styleName") }
            for variation in component.variations where variation.reference.isEmpty {
                issues.append("\(component.key): вариация \(variation.name) без reference")
            }
        }
        return issues
    }
...
extension ComponentBindingInfo {
    func componentMeta(includeStyleApi: Bool) -> ConfigInfo.ComponentMeta {
        ...
        return .init(
            ...
            styleName: component,
            props: props,
            styleApi: includeStyleApi
                ? .init(stylesClassName: stylesEnumName, returnTypeName: returnType, params: params)
                : nil,
