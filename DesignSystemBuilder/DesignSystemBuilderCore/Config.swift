...
public struct DesignSystemBuilderConfiguration: Codable {
    ...
    public struct ThemeConfiguration: Codable {
        ...
        /// Если указан, `DesignSystemBuilder` использует `file://`-URL вместо
    ...
    /// Описание тенанта на стороне dsbuilder: имя + URL дополнительной zip-схемы.
...
public extension DesignSystemBuilderConfiguration.Theme {
    var themeConfiguration: DesignSystemBuilderConfiguration.ThemeConfiguration {
        switch self {
        case .sddsServTheme:
            .init(
                name: self.rawValue,
    ...
    private func themeURL(name: String, file: String = "latest.zip") -> String {
        "\(DesignSystemBuilderConfiguration.Theme.baseURL)/\(name)/\(file)"
    }
}
