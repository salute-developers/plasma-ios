...
public struct SDDSTabItem<StartContent: View, ContentRight: View, ActionContent: View>: View {
    ...
    @ViewBuilder private var horizontalLayout: some View {
        VStack(spacing: 0) {
            if isIconTabItem {
                // IconTabItem Layout - icon with counter overlay
                iconTabItemContent
                ...
                HStack(alignment: .center, spacing: 0) {
                    ...
                    if !(startContent is EmptyView) {
                        startContent
                            ...
                            .padding(.trailing, appearance.size.startContentPadding)
                    }
                    ...
                    // Content Right (Counter или Custom View)
                    if !(contentRight is EmptyView) {
                        contentRight
                            .padding(.leading, appearance.size.counterPadding)
                    } else if let counterValue = counterValue, let counterAppearance = appearance.counterAppearance {
                        SDDSCounter(
                            text: "\(counterValue)",
    ...
    /// Определяет, является ли это IconTabItem (только иконка без label)
    private var isIconTabItem: Bool {
        return label.isEmpty
    }
    
    /// Layout для IconTabItem - иконка с counter в правом верхнем углу
