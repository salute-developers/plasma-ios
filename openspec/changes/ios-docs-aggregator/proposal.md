...
- `SDDSThemeBuilder` генерирует два info-артефакта при генерации темы:
...
- `docs-info-artifacts`: генерация `config-info-swiftui.json` и `theme-info.json` в
  `SDDSThemeBuilder` по golden-контракту backend (`InfoArtifactDtos`, фикстуры
...
- `SDDSThemeBuilder/SDDSThemeBuilderCore` — новые генераторы info-артефактов, интеграция в
