...
1. **Info-артефакты генерирует `DesignSystemBuilder`** (не отдельный тул поверх
...
2. **Экстрактор сэмплов и агрегатор — один SwiftPM-бинарь `Tools/SDDSDocsAggregator`**
   (SwiftSyntax). SwiftSyntax нельзя тянуть в xcodebuild-граф `DesignSystemBuilder` (ломает
