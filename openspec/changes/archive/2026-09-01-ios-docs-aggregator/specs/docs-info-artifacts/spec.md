...
Генерация машинных info-артефактов документации в `DesignSystemBuilder` по golden-контракту
backend Documentation Service (`InfoArtifactDtos`, фикстуры
`ios-components.json`/`ios-theme.json`; форматы `sdds-swiftui-components-info-v1` /
`sdds-ios-theme-info-v1`). Файлы: per-theme `Themes/<Name>Theme/.sdds/config-info-ios.json`
(components-info) и `config-info-tokens-ios.json` (theme-info) — существующие выходы
theme-builder, доведённые до полного покрытия; агрегатор копирует их в
`meta/components-info.json` / `meta/theme-info.json`.

## ADDED Requirements

### Requirement: Генерация components-info для платформы swiftui
`DesignSystemBuilder` SHALL при генерации компонентов темы записывать per-theme
`.sdds/config-info-ios.json` со структурой `{name, packageName, components:[...]}`, где
...
`DesignSystemBuilder` SHALL при генерации токенов темы записывать per-theme
`.sdds/config-info-tokens-ios.json` со
...
`DesignSystemBuilder` MUST валидировать выход против структуры golden-фикстур
