...
get_changelog_url() {
    ...
    # На Android: current/xml/artifact-id/changelog.json для main
    # На iOS: current/ios/theme-name/changelog.json для main
...
docusaurus_changelog_deploy() {
    ...
    if s3cmd put "$CHANGELOG_JSON_PATH" "s3://$S3_BUCKET/$changelog_url" \
        ...
        log "Public URL: $S3_ENDPOINT/$S3_BUCKET/$changelog_url"
    else
