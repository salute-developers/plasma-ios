...
- `release/build_release.sh <tag>` — все ассеты релиза в `release-artifacts/`: SDDSThemeCore,
  InputMask, SDDSComponents, SDDSIcons, темы, `dsbuilder-cli-<tag>.zip`, `SDDSSources-<tag>.zip`.
...
- `release_sync/`, `build-release-announcement.sh` — релизные шаги.
- Публикация релиза — один прогон `publish-release.yml` (workflow_dispatch на `main`):
  `release/build_release.sh` → тег → draft-релиз → upload ассетов → снятие draft. Вход `dry_run`
  собирает ассеты без тега и релиза — так воркфлоу проверяют с любой ветки.
